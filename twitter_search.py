...
# Parse the command
    ...
    parser.add_argument('-c', action='store', dest='c', help='Tweet count (must be <350000)')
...
def tw_search_json(query, cnt=5):
    ...
    data = []
    for tweet in tweepy.Cursor(api.search, q=query, count=cnt, since = date_since).items():
        dTwt = {}
